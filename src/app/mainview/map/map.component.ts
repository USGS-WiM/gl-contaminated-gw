...
export class MapComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._mapService.defaultAuxLayers.forEach((layerName) =>
      this._mapService.map.addLayer(this._mapService.auxLayers[layerName])
    );
  ...
  //could probably be delete but I don't know if any other code calls this; expecially in the component template
  resizeMap() {
    setTimeout(() => {
      this._mapService.map.invalidateSize();
    }, 150);
